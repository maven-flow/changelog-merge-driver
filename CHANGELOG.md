...
- A version name written as an inline link (`## [1.0.0](https://example.com/compare/v0.9.0...v1.0.0) - 2019-02-15`) lost its link when the changelog was written back. The link is now carried through merge, rebase and no-base modes. Where both sides changed the link of a released version, `theirs` wins, as for the rest of released history. (Reference-style link definitions at the end of the file, the form [Keep a Changelog](https://keepachangelog.com/) itself uses, are not yet carried through a merge.)
